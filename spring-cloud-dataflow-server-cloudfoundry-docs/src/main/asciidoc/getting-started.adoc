...
NOTE: You must use a unique name for your app. An app with the same name in the same organization causes your
deployment to fail.
...
[NOTE]
=====
If you are going to use `Skipper` to deploy Streams, deploy Skipper first and then configure the URI location where the Skipper server runs and set the feature toggle to use skipper
