...
[subs=attributes]
```
cf push dataflow-server -m 2G --no-start -p spring-cloud-dataflow-server-cloudfoundry-{project-version}.jar
cf bind-service dataflow-server redis
cf bind-service dataflow-server my_mysql
```

IMPORTANT: The recommended minimal memory setting for the server is 2G. Also, to obtain extra runtime information about
