...
Data Flow for Cloud Foundry. This is mainly targeted at non-java apps, as when using Docker the auto-reconfiguration
magic offered by the java buildpack will not kick in (your Docker app would then not automatically bind to services
unless it was custom built to include auto-reconfiguration connectors). You can of course still bind services to the
deployed apps, but those will have to parse `VCAP` variables and Spring Cloud Stream / Spring Cloud Task variables
"by themselves".
