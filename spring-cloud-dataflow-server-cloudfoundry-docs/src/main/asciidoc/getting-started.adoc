...
```
cf set-env dataflow-server SPRING_APPLICATION_JSON '{"maven": { "remote-repositories": { "repo1": { "url": "https://repo.spring.io/libs-snapshot", "auth": { "username": "user1", "password": "pwd"} }, "repo2": { "url": "https://repo2.spring.io/libs-snapshot"} }, "proxy": {"protocol":"https", "host": "proxy-host1", "port": "9080", "auth": {"username": "proxy-user1", "password": "proxy-pwd"} } } }'
```
where `repo1` and `repo2` are the alias names for the remote repositories.
...
- name: {PREFERRED NAME OF THE SERVER APP}
  ...
  env:
    ...
    SPRING_APPLICATION_JSON {"maven": { "remote-repositories": { "repo1": { "url": "https://repo.spring.io/libs-snapshot", "auth": { "username": "user1", "password": "pwd"} }, "repo2": { "url": "https://repo2.spring.io/libs-snapshot"} }, "proxy": {"protocol":"https", "host": "proxy-host1", "port": "9080", "auth": {"username": "proxy-user1", "password": "proxy-pwd"} } } }
...
If you're using App Starters, your application already embeds the `spring-cloud-services-starter-config-client` dependency.
If you're building your application from scratch, simply add a reference to that dependency. A maven example snippet follows:
